...
def generateState(input):
    state = {}
    for aunt in input:
        name, number, thing1, thing1Val, thing2, thing2Val, thing3, thing3Val = aunt.split(" ")
        number = number.replace(":", "")
        name = name + number
        state[name] = {}
        state[name][thing1.replace(":", "")] = int(thing1Val.replace(",", ""))
        state[name][thing2.replace(":", "")] = int(thing2Val.replace(",", ""))
        state[name][thing3.replace(":", "")] = int(thing3Val.replace(",", ""))
    return state

def generateGiftState(gift):
    state = {}
    for item in gift:
        name, value = item.split(" ")
        name = name.replace(":", "")
        state[name] = int(value.replace(",", ""))
    return state

def findMatch(state, gift):
    scores = []
    for aunt in state:
        auntScore = 0
        for item in gift:
            if item in state[aunt]:
                if item in ["cats", "trees"] and state[aunt].get(item) > gift.get(item):
                    auntScore += 1
                elif item in ["pomeranians", "goldfish"] and state[aunt].get(item) < gift.get(item):
                    auntScore += 1
                elif gift.get(item) == state[aunt].get(item):
                    auntScore += 1

        if auntScore > 0:
            scores.append(str(auntScore) + " " + aunt)

    scores.sort()
    print scores

def solve(input, gift):
    state = generateState(input)
    gift = generateGiftState(gift)
    findMatch(state, gift)
