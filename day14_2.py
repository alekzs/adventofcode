...
def solve(input):
    ...
    for i in range(2505):
        ...
        # Award leading reindeer
        leadingDistance = max(list(map(lambda (k, v): v["distanceTravelled"], state.iteritems())))
        for deerName in state:
            if state[deerName]["distanceTravelled"] == leadingDistance:
                state[deerName]["extraPoints"] += 1
    print max(list(map(lambda (k, v): v["extraPoints"], state.iteritems())))
