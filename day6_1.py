def countOn(table):
    ...
    for x in xrange(len(table)):
        for y in xrange(len(table[x])):
            if table[x][y]:
                numOn += 1
    print numOn


def do(table, state, coordinateStart, coordinateEnd):
    startX = int(coordinateStart.split(",")[0])
    startY = int(coordinateStart.split(",")[1])
    endX = int(coordinateEnd.split(",")[0])
    endY = int(coordinateEnd.split(",")[1])
...
def solve(input):
    table = [[False for x in xrange(1000)] for y in xrange(1000)]
    #print table
    #input = input[:10]
    for instruction in input:
        split = instruction.split(" ")
        state = None
        coordinateStart = None
        coordinateEnd = None
        if split[0] == "toggle":
            state = "toggle"
            coordinateStart = split[1]
            coordinateEnd = split[3]
        else:
            state = split[1]
            coordinateStart = split[2]
            coordinateEnd = split[4]
        do(table, state, coordinateStart, coordinateEnd)
    countOn(table)
